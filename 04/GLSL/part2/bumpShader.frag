...
//
// Bump fragment shader GLSL part 2 for Lab 4.
//
...
{   
    ...
    // noise frequency (ml3406)
    float frequency = 45;

    // generate a bump map using noise based on position (ml3406)
    // offsets in x and y directions
    float dx = 0.01;
    float dy = 0.01;
    // use 4 adjacent positions to calculate gradients in tangent and bitangent directions (ml3406)
    float top = noise(vec3(pos.x, pos.y + dy, pos.z) * frequency);
    float left = noise(vec3(pos.x - dx, pos.y, pos.z) * frequency);
    float right = noise(vec3(pos.x + dx, pos.y, pos.z) * frequency);
    float bottom = noise(vec3(pos.x, pos.y - dy, pos.z) * frequency);
    
    // make height range from mini to maxi for adjusting average bump height (ml3406)
    float maxi = 0.24;
    float mini = 0.0;
    ...
    // gradients in tangent (x) and bitangent (y) directions (ml3406)
    float tan_gradient = (right - left);
    float bitan_gradient = (top - bottom);
    
    // new normal from the bump map
    vec3 norm = normalize(cross(vec3(1.0, 0.0, 0.0) * (1.0 - abs(tan_gradient)) + vec3(0.0, 0.0, 1.0) * tan_gradient, vec3(0.0, 1.0, 0.0) * (1.0 - abs(bitan_gradient)) + vec3(0.0, 0.0, 1.0) * bitan_gradient));
    ...
    if(sb < MORTARTHICKNESS || tb < MORTARTHICKNESS) {
        ...
        // mortar (ml3406)
            ...
            colorTemp = mortarColor;
            
            // modify N to create groove (ml3406)
            // radius of the groove
            float r = MORTARTHICKNESS / 2.0;

            // distances to the groove center in s and t directions (ml3406)
            float ds = 0.0;
            float dt = 0.0;

            // tangent vecotrs in s and t directions for modifying N (ml3406)
            vec3 tangentS = vec3(0.0);
            vec3 tangentT = vec3(0.0);
            
            // intersection mortar (ml3406)
            if(sb < MORTARTHICKNESS && tb < MORTARTHICKNESS){
                ds = sb - r;
                dt = tb - r;
                tangentS = vec3(-1.0, 0.0, 0.0);
                tangentT = vec3(0.0, -1.0, 0.0);
            }
            // mortar in columns (ml3406)
            else if(sb < MORTARTHICKNESS) {
                ds = sb - r;
                tangentS = vec3(-1.0, 0.0, 0.0);
            } 
            // mortar in rows (ml3406)
            else {
                dt = tb - r;
                tangentT = vec3(0.0, -1.0, 0.0);
            }
            
            // new normal at the groove (ml3406)
            norm = normalize(N * cos(asin(abs(ds) / r)) * cos(asin(abs(dt) / r)) + tangentS * ds / r + tangentT * dt / r);
        }
    } 
    // tiles (ml3406)
    else {   
    ...
    // convert to tangent space (ml3406)
    float temp_x = vTan.x * norm.x + vTan.y * norm.y + vTan.z * norm.z;
    float temp_y = vBitan.x * norm.x + vBitan.y * norm.y + vBitan.z * norm.z;
    float temp_z = N.x * norm.x + N.y * norm.y + N.z * norm.z;
     
    // manipulate N and recalculate R (ml3406)
    N = normalize(vec3(temp_x, temp_y, temp_z));
