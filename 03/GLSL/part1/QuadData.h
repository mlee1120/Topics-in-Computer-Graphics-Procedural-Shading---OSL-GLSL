...
//
static float quadNormals[] = { 0.0, 0.0, 1.0 };

static int quadNormalsLength = sizeof(quadNormals) / sizeof(float);

//
// Each vertex defining the quad is present exactly once in the vertex
// array.  This means that the element array isn't just a sequential list
// of vertex indices; instead, it must explicitly list the three vertex
// indices which define each triangle.
//
