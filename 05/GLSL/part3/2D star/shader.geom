...
layout(points) in;
layout(triangle_strip, max_vertices=92) out; // use triangle strip (ml3406)
...
void main()
{
    ...
    // finish last triangle (ml3406)
    gl_Position = gl_in[0].gl_Position;
    EmitVertex();
    gl_Position = gl_in[0].gl_Position + vec4(0.0, radius[0], 0.0, 0.0);
    EmitVertex();
