#version 150 core
//
// Geometry shader for Lab 5 part 1.
//
...
layout(points) in;
layout(points, max_vertices=92) out;
...
void main()
{
    ...
    for (int i = 0; i < factor[0] + 1; i++) {
        for (int j = 0; j < factor[0]; j++) {
            ...
            vec3 new_point = rotate_quaternion(vec4(point, 0.0), vec3(0.9, 0.0, -0.3), 30 * PI / 180);
            
            // emit a single point (ml3406)
            gl_Position = gl_in[0].gl_Position + vec4(new_point, 0.0);
            EmitVertex();
