...
static const char *textures[N_SHAPES] = {
    "star.jpg",              // Cube     changed to star.jpg (ml3406) 
    "panorama-1820x1028.jpg",// Cylinder
    "tennisball-used.jpg"    // Sphere   changed to tennisball-used.jpg (ml3406)
};

// texture bump file name (ml3406)
static const char *texture_bump = "tennisball-bump.jpg";

// Texture IDs
GLuint tids[N_SHAPES];

// texture bump id (ml3406)
GLuint tbid;
...
static GLuint load_texture( const char *file ) {
    ...
    // I used SOIL_LOAD_RGB because useing AUTO leads to Segmentation fault (star.jpg is a grayscale file) (ml3406)
    image = SOIL_load_image( file, &width, &height, &channels, SOIL_LOAD_RGB);
...
{   
    // enable blend for tuning opacity (ml3406)
    glEnable(GL_BLEND);
    ...
    // set up second texture (ml3406)
    glActiveTexture(GL_TEXTURE0 + 3);
    tbid = load_texture(texture_bump);
}
...
void setTextures( GLuint program, Shape obj )
{
    ...
    // second smapler (ml3406)
    loc = getUniformLoc(program, "bump");
    unit = 3;
    if(loc >= 0) {
        glActiveTexture(GL_TEXTURE0 + unit);
